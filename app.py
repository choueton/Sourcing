...
def add_from_promo():
    ...
    return render_template(
        "add_from_promo.html", formation=formation, bailleur=bailleur
    )
...
def add_candidat():
    ...
    cur.execute(
        "INSERT INTO candidat (nom, prenom, email, telephone, genre, nationalite, date_naissance, lieu_residance, ville, statut_sociale, diplome_actuel, specialite_etude, ecole, id_promo, contrainte, source, decision_finale) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
        (
            nom,
            prenom,
            email,
...
def add_bailleur():
    ...
    cur = mysql.connection.cursor()
    cur.execute(
        "INSERT INTO bailleur (nom_bailleur, description) VALUES (%s,%s)",
        (nom_bailleur, description),
    )
    mysql.connection.commit()
    cur.close()
...
def upd_bailleur(id):
    ...
    if request.method == "POST":
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            "UPDATE bailleur SET nom_bailleur=%s, description=%s WHERE id=%s",
            (nom_bailleur, description, id),
        )
        mysql.connection.commit()
        cur.close()
        return redirect(url_for("list_bailleur"))
...
def add_formation():
    ...
    cur = mysql.connection.cursor()
    cur.execute(
        "INSERT INTO formation (nom_formation, duree) VALUES (%s,%s)",
        (nom_formation, duree),
    )
    mysql.connection.commit()
    cur.close()
...
def upd_formation(id):
    ...
    if request.method == "POST":
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            "UPDATE formation SET nom_formation=%s, duree=%s WHERE id=%s",
            (nom_formation, duree, id),
        )
        mysql.connection.commit()
        cur.close()
        return redirect(url_for("list_formation"))
...
def add_promo():
    ...
    cur.execute(
        "INSERT INTO promo (nom_promo, id_bailleur, id_formation, debut_promo, fin_promo) VALUES (%s, %s, %s, %s, %s)",
        (nom_promo, id_bailleur, id_formation, debut_promo, fin_promo),
    )
...
def upd_promo(id):
    cur = mysql.connection.cursor()
    cur.execute(
        "SELECT promo.id, promo.nom_promo, bailleur.nom_bailleur, formation.nom_formation, promo.debut_promo, promo.fin_promo FROM bailleur JOIN promo ON bailleur.id = promo.id_bailleur JOIN formation ON formation.id = promo.id_formation WHERE promo.id=%s",
        (id,),
    )
    promo_info = cur.fetchall()
    cur.execute("SELECT * FROM bailleur")
    ...
    if request.method == "POST":
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            "UPDATE promo SET nom_promo=%s, id_bailleur=%s, id_formation=%s, debut_promo=%s, fin_promo=%s WHERE id=%s",
            (nom_promo, id_bailleur, id_formation, debut_promo, fin_promo, id),
        )
        mysql.connection.commit()
        cur.close()
        return redirect(url_for("list_promo"))
    ...
    return render_template(
        "upd_promo.html", promo_info=promo_info, bailleur=bailleur, formation=formation
    )
...
def add_simplonien():
    ...
    cur.execute(
        "INSERT INTO simplonien (nom, prenom, email, telephone, id_promo, tuteur, residance) VALUES (%s, %s, %s, %s, %s, %s, %s)",
        (nom, prenom, email, telephone, id_promo, tuteur, residance),
    )
...
def upd_simplonien(id):
    ...
    if request.method == "POST":
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            "UPDATE simplonien SET nom=%s, prenom=%s, email=%s, telephone=%s, tuteur=%s, residance=%s WHERE id=%s",
            (nom, prenom, email, telephone, tuteur, residance, id),
        )
        mysql.connection.commit()
        cur.close()
        return redirect(url_for("list_simplonien"))
